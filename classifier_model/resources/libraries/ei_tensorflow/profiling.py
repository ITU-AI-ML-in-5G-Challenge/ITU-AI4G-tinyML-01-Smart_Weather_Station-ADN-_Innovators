...
from ei_shared.metrics_utils import allocate_to_bins, MetricsJson
...
def profile_model(model_type, model, model_file, validation_dataset, Y_test, X_samples, Y_samples,
    ...
    if mode == 'classification':
        ...
        if objdet_details.last_layer != 'fomo':
            ...
            if class_names is not None:
                metrics['class_names'] = ['_background'] + class_names
...
def get_model_metadata(keras_model, validation_dataset, Y_test, X_samples, Y_samples, has_samples,
                       ...
                       metrics_fname: str=None, curr_metadata_mtime:int = None):
    ...
    if keras_model:
        ...
        if (
            ...
            and (curr_metadata_mtime is None or curr_metadata_mtime > 1710749753)
        ):
    ...
    # be rerun.
    metrics_json = MetricsJson(metrics_fname, reset=True)
